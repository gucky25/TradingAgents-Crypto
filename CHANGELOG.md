...
- Crypto-first analysis mode with `instrument_type="crypto"` as the default.
- Crypto-aware vendor routing through `tradingagents/dataflows/crypto.py` for OHLCV, indicators, and fundamentals guidance.
- Crypto-specific prompt context for analyst, researcher, trader, risk, and portfolio-manager agents.
- Configurable crypto benchmark via `crypto_benchmark` for memory-log outcome reflection.
- CLI prompts for instrument type, crypto symbol defaults, and crypto benchmark.
- `docs/CRYPTO_USAGE.md` with CLI and Python usage instructions.
- `ActionReport.md` for project decisions, actions, and implementation notes.

### Changed

- Package metadata now identifies the project as a crypto market analysis framework.
- Default data vendors route price, technical, and fundamentals requests through crypto-aware wrappers before falling back to yfinance.
- Fundamentals analysis no longer assumes company statements when crypto mode is enabled.
- Reflection prompt now uses benchmark-relative return wording instead of hardcoded SPY language.

### Notes

- This release is still an analysis/signaling system, not a live trading or exchange execution bot.
